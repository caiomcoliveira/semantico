...
symbol *tabela_symbols = (symbol*)malloc(sizeof(symbol));

error *errors= (error *)malloc(sizeof(error));
error *warnings= (error *)malloc(sizeof(error));
...
void push_symbol_type(char* tipo, int line);
...
simbolo*  get_symbol( char * sym_name){
	simbolo *table = symbol_table;
	while(table!= NULL){		
...
void print_symbol_table(){
  ...
  printf("_______________________________________________________\n");
	...
	while(table!= NULL){		
		printf("|%s\t\t |%s \t\t |%s\n", table->name, table->tipo, (table->used == 0) ? "NO" : "YES");
		table = table->next;
	}
  printf("---------------------------------------------------------\n");
}
...
void push_symbol_type(char* tipo, int n){
		simbolo * table = symbol_table;
		while(table!= NULL){		
			if(table->line == n){
				table->tipo = strdup(tipo);
			}
			table = table->next;
		}
}